...
class MySubscribeCallback(SubscribeCallback):
    ...
    def message(self, pubnub, message):
        ...
        if counter == 1:
            ...
            condition = True
            while condition:
                blk = json.dumps({'Block number': counter, 'Creator': "Alice", 'Hash': prehash, 'Transaction': transactions[counter - 1], 'Nonce': nonce}, sort_keys=True, indent=4, separators=(',', ': '))
            ...
            fw.close()
            print("Alice write file " + str(counter))
            ...
            if int(hashval, 16) == int(prehash, 16):
                ...
                if counter < len(transactions) or counter == len(transactions):
                    ...
                    fw.close()
                    print("Alice write file " + str(counter))
                    pubnub.publish().channel('Channel-Bob').message(blk).sync()
                else:
                    print("Done!")
            else:
...
pubnub.publish().channel('Channel-Bob').message('1').sync()
